...
TEXT = {
    ...
    "provider_help": {"zh": "deepseek 使用 DeepSeek API；openai-compatible 使用统一 LLM 配置；none 禁用模型；echo 仅供开发", "ja": "deepseek は DeepSeek API を使用します。openai-compatible は統一 LLM 設定を使用し、none はモデルを無効化、echo は開発専用です", "en": "deepseek uses the DeepSeek API; openai-compatible uses the unified LLM config; none disables the model; echo is development-only"},
    ...
    "llm_key_missing": {"zh": "警告：所选 provider 未配置可用密钥；设置 PCUI_DEEPSEEK_API_KEY 或 PCUI_LLM_API_KEY 后再启用模型接口。", "ja": "警告：選択した provider の利用可能なキーがありません。PCUI_DEEPSEEK_API_KEY または PCUI_LLM_API_KEY を設定してください。", "en": "WARNING: no usable key is configured for the selected provider; set PCUI_DEEPSEEK_API_KEY or PCUI_LLM_API_KEY."},
