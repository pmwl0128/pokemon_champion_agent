...
TEXT = {
    ...
    "provider_help": {"zh": "opencode 使用 OpenCode Go，并在不可用时回退 DeepSeek；openai-compatible 使用统一 LLM 配置；none 禁用模型；echo 仅供开发", "ja": "opencode は OpenCode Go を使用し、利用不可時は DeepSeek にフォールバックします。openai-compatible は統一 LLM 設定を使用、none はモデル無効、echo は開発専用です", "en": "opencode uses OpenCode Go with DeepSeek fallback; openai-compatible uses the unified LLM config; none disables the model; echo is development-only"},
    ...
    "llm_key_missing": {"zh": "警告：所选 provider 未配置可用密钥；设置 PCUI_OPENCODE_API_KEY、PCUI_DEEPSEEK_API_KEY 或 PCUI_LLM_API_KEY 后再启用模型接口。", "ja": "警告：選択した provider の利用可能なキーがありません。PCUI_OPENCODE_API_KEY、PCUI_DEEPSEEK_API_KEY、または PCUI_LLM_API_KEY を設定してください。", "en": "WARNING: no usable key is configured for the selected provider; set PCUI_OPENCODE_API_KEY, PCUI_DEEPSEEK_API_KEY, or PCUI_LLM_API_KEY."},
