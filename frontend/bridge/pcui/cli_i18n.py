...
TEXT = {
    ...
    "provider_help": {"zh": "mimo 使用 MiMo Anthropic API，并在不可用时回退 DeepSeek Anthropic API；none 禁用模型；echo 仅供开发", "ja": "mimo は MiMo Anthropic API を使用し、利用不可時は DeepSeek Anthropic API にフォールバックします。none はモデルを無効化、echo は開発専用です", "en": "mimo uses the MiMo Anthropic API with DeepSeek Anthropic fallback; none disables the model; echo is development-only"},
    ...
    "llm_key_missing": {"zh": "警告：所选 provider 未配置可用密钥；设置 PCUI_MIMO_API_KEY、PCUI_DEEPSEEK_API_KEY 或 PCUI_LLM_API_KEY 后再启用模型接口。", "ja": "警告：選択した provider の利用可能なキーがありません。PCUI_MIMO_API_KEY、PCUI_DEEPSEEK_API_KEY、または PCUI_LLM_API_KEY を設定してください。", "en": "WARNING: no usable key is configured for the selected provider; set PCUI_MIMO_API_KEY, PCUI_DEEPSEEK_API_KEY, or PCUI_LLM_API_KEY."},
