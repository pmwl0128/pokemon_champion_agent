...
- 计算页各工具共享一份队伍与战场（`roster.tsx`）：双方队伍与配置、出场位、各方所选招式、赛制/天气/场地/双方场地状态。A 方恒为我方、B 方为对方；攻守互换交换整份数据而不是翻转某个工具的视角。各工具只读取自身计算能保留的条件，其余条件留在共享战场里、不进入该工具的数字；工具自有的读数状态（乱数档、会心、耐久目标）不进共享层。每只宝可梦有稳定 `uid`，跨工具的引用（耐久目标、还原点、自动填充）都按 uid 而非下标；
