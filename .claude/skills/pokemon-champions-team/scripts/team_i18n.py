...
MESSAGES: dict[str, dict[str, str]] = {
    ...
    'role_label_terrain_control': {"zh": '场地控制', "ja": 'フィールド管理', "en": 'terrain control'},
    ...
    'role_label_disruption': {"zh": '干扰', "ja": '妨害', "en": 'disruption'},
    'role_label_damage_mitigation': {"zh": '火力削弱', "ja": '火力削減', "en": 'damage mitigation'},
    'role_label_priority_attack': {"zh": '先制攻击', "ja": '先制技', "en": 'priority attack'},
    'role_label_weather_rewrite': {"zh": '天气改写', "ja": '天候書き換え', "en": 'weather rewrite'},
    'role_label_fake_out': {"zh": '击掌奇袭（双打）', "ja": 'ねこだまし（ダブル）', "en": 'Fake Out pressure (doubles)'},
    'role_label_spread': {"zh": '范围输出（双打）', "ja": '全体攻撃（ダブル）', "en": 'spread damage (doubles)'},
    ...
    'role_signal_seed_sower': {"zh": '受到攻击时开启青草场地', "ja": '攻撃を受けるとグラスフィールドを展開', "en": 'Grassy Terrain setter (on being hit)'},
    ...
    'role_signal_grassy_seed': {"zh": '青草场地下防御+1（一次性）', "ja": 'グラスフィールドで防御+1（1回）', "en": 'Def +1 on Grassy Terrain (one-time)'},
    'role_signal_electric_seed': {"zh": '电气场地下防御+1（一次性）', "ja": 'エレキフィールドで防御+1（1回）', "en": 'Def +1 on Electric Terrain (one-time)'},
    'role_signal_psychic_seed': {"zh": '精神场地下特防+1（一次性）', "ja": 'サイコフィールドで特防+1（1回）', "en": 'SpD +1 on Psychic Terrain (one-time)'},
    'role_signal_misty_seed': {"zh": '薄雾场地下特防+1（一次性）', "ja": 'ミストフィールドで特防+1（1回）', "en": 'SpD +1 on Misty Terrain (one-time)'},
    'role_signal_terrain_extender': {"zh": '场地持续回合由 5 延长到 8', "ja": 'フィールドの持続が5→8ターン', "en": 'terrain lasts 8 turns instead of 5'},
    'role_signal_air_balloon': {"zh": '受到攻击前免疫地面属性', "ja": '攻撃を受けるまで地面タイプを無効', "en": 'Ground immunity until hit'},
    'role_signal_eject_button': {"zh": '受到伤害时自动换下（一次性）', "ja": 'ダメージを受けると交代（1回）', "en": 'switches out when hit (one-time)'},
    'role_signal_red_card': {"zh": '强制攻击方换下（一次性）', "ja": '攻撃してきた相手を強制交代（1回）', "en": 'forces the attacker out (one-time)'},
    'role_signal_leek': {"zh": '大葱鸭／葱游兵易击要害+2', "ja": 'カモネギ・ネギガナイトの急所ランク+2', "en": 'crit rate +2 for the Farfetch\'d line'},
    'role_signal_normal_gem': {"zh": '一般属性招式威力 ×1.3（一次性）', "ja": 'ノーマル技の威力1.3倍（1回）', "en": 'x1.3 Normal move (one-time)'},
    'role_signal_binding_band': {"zh": '束缚伤害由 1/8 提升为 1/6', "ja": 'バインドダメージが1/8→1/6', "en": 'binding damage 1/6 instead of 1/8'},
    'role_signal_guard_dog': {"zh": '免疫威吓（改为攻击+1），且不会被强制换下', "ja": 'いかくを無効化して攻撃+1、強制交代も無効', "en": 'immune to Intimidate (Attack +1 instead); cannot be forced out'},
    'role_signal_rattled': {"zh": '受恶／幽灵／虫属性招式或威吓时速度+1', "ja": 'あく・ゴースト・むし技やいかくで素早さ+1', "en": 'Speed +1 when hit by Dark/Ghost/Bug or intimidated'},
    'role_signal_punk_rock': {"zh": '声音招式威力 ×1.3，受到的声音伤害减半', "ja": '音技の威力1.3倍、受ける音技のダメージ半減', "en": 'x1.3 sound moves, halves incoming sound damage'},
    'role_signal_stakeout': {"zh": '攻击刚换上场的对手时威力 ×2', "ja": '交代で出てきた相手への威力2倍', "en": 'x2 power against a foe switching in'},
    'role_signal_steely_spirit': {"zh": '自己与同伴的钢属性招式威力 ×1.5', "ja": '自分と味方のはがね技の威力1.5倍', "en": 'x1.5 Steel moves for itself and its ally'},
    'role_signal_thermal_exchange': {"zh": '受火属性招式时攻击+1，且不会灼伤', "ja": 'ほのお技を受けると攻撃+1、やけどにならない', "en": 'Attack +1 when hit by Fire; cannot be burned'},
    'role_signal_liquid_ooze': {"zh": '吸取类招式改为对使用者造成伤害', "ja": '吸収技は回復させず相手にダメージ', "en": 'drain moves damage the drainer instead of healing it'},
    'role_signal_grass_pelt': {"zh": '青草场地下防御 ×1.5', "ja": 'グラスフィールドで防御1.5倍', "en": 'Defense x1.5 on Grassy Terrain'},
    'role_signal_aura_guard': {"zh": '接触类招式造成的伤害减半', "ja": '接触技のダメージ半減', "en": 'halves damage from contact moves'},
    'role_signal_emergency_exit': {"zh": 'HP 降到一半以下时强制回队', "ja": 'HPが半分以下になると控えに戻る', "en": 'forced back to the party below half HP'},
    'role_signal_run_away': {"zh": '无视妨碍替换的效果进行替换', "ja": '交代を妨げる効果を無視して交代できる', "en": 'switches out through trapping effects'},
    'role_signal_libero': {"zh": '每次出场时首个招式会改变自身属性', "ja": '登場ごとに最初に使う技のタイプへ変化', "en": 'becomes the type of the move it uses (once per entry)'},
    ...
    'frame_unpartitioned': {"zh": '全池（结构上未分组）', "ja": '全体（構造では分割せず）', "en": 'whole pool (did not separate structurally)'},
    'frame_substitutable': {"zh": '可替换位（recurring-partner）', "ja": '差し替え可能枠（recurring-partner）', "en": 'substitutable slots (recurring-partner)'},
    'frame_fallback_history': {"zh": '兜底骨架（旧规则）：当前规则没有任何真实队伍带这只锚点，以下骨架来自旧规则中围绕它组建的队伍；偏离只记录不淘汰', "ja": 'フォールバック骨組み（旧ルール）：現ルールの実チームにこのアンカーがないため、旧ルールでこのポケモンを軸にしたチームから骨組みを取っています。逸脱は記録のみで除外しません', "en": 'FALLBACK (older rule): no current-rule team carries this anchor; the skeleton comes from older-rule teams built around it. Departures are reported, never eliminated.'},
    'frame_fallback_partners': {"zh": '兜底骨架（特征队友）：当前规则没有任何真实队伍带这只锚点，以下骨架取自同时带有它的特征使用率队友的真实队伍；偏离只记录不淘汰', "ja": 'フォールバック骨組み（特徴的パートナー）：現ルールの実チームにこのアンカーがないため、特徴的な使用率パートナーを同時に含む実チームから骨組みを取っています。逸脱は記録のみで除外しません', "en": 'FALLBACK (specific partners): no current-rule team carries this anchor; the skeleton comes from real teams carrying its specific usage partners together. Departures are reported, never eliminated.'},
    'frame_fallback_stand_in': {"zh": '兜底骨架（角色替身）：当前规则没有任何真实队伍带这只锚点，以下骨架借自一只承担相同职能、有真实数据的宝可梦，锚点顶替其位置；偏离只记录不淘汰', "ja": 'フォールバック骨組み（代役）：現ルールの実チームにこのアンカーがないため、同じ役割を担うデータのあるポケモンの骨組みを借り、アンカーをその枠に置いています。逸脱は記録のみで除外しません', "en": 'FALLBACK (stand-in): no current-rule team carries this anchor; the skeleton is borrowed from a data-backed species doing the same job, with the anchor in its place. Departures are reported, never eliminated.'},
    'frame_fallback_generic_library': {"zh": '兜底骨架（通用骨架）：锚点既没有真实队伍也没有使用率队友，以下是整个库的通用骨架加上锚点；主轴不是围绕锚点观察到的，偏离只记录不淘汰，回答中必须说明骨架来源', "ja": 'フォールバック骨組み（汎用）：アンカーには実チームも使用率パートナーもないため、ライブラリ全体の汎用骨組みにアンカーを加えています。主軸はアンカーと共に観測されたものではなく、逸脱は記録のみで除外しません。回答で骨組みの出所を明記してください', "en": 'FALLBACK (generic library): the anchor has neither real teams nor usage partners; these are whole-library skeletons with the anchor added. Their backbone was not observed with the anchor, so departures are reported, never eliminated — say where the frame came from in the answer.'},
    'frame_empty_backbone': {"zh": '空骨架：组内没有任何宝可梦共现率达到 {bar}，本框架只给出结构事实与开放位，没有可直接搭建的主轴。置信度因此为 low——不要把开放位当作自由发挥，每一只都要用 repset/search 单独取得依据并在回答中说明骨架为空', "ja": '骨組みが空です：このグループ内で共起率 {bar} に達するポケモンが存在せず、構造の事実と空き枠だけが得られています。信頼度が low なのはそのためです。空き枠を自由枠と読み替えず、各メンバーを repset/search で個別に裏付け、骨組みが空だったことを回答に明記してください', "en": 'EMPTY BACKBONE: no species recurs in >= {bar} of this group, so the frame supplies structural facts and open slots but no roster to build on. That is why confidence is low — do not read the open slots as freedom: ground every member through repset/search and say the backbone was empty.'},
    'frame_no_backbone': {"zh": '所有展示框架都没有主轴：没有任何结构组中存在共现率 ≥ {bar} 的宝可梦，本次运行不提供有数据依据的起始阵容。整体置信度为 low；逐只用 repset/search 取证并披露，不要用模型先验填满空位', "ja": '表示されたすべてのフレームに主軸がありません：どの構造グループにも共起率 {bar} 以上のポケモンがなく、今回はデータ根拠のある初期構成を提示できません。全体の信頼度は low です。各メンバーを repset/search で裏付けて開示し、モデルの事前知識で空き枠を埋めないでください', "en": 'NO BACKBONE in any shown frame: no species recurs in >= {bar} of any structural group, so this run hands you no grounded starting roster. Overall confidence is low — ground each member through repset/search and disclose it; do not fill the slots from the training prior.'},
    'frame_structure_unobserved': {"zh": '结构未观测：这些队伍行是仅记录物种的来源数据（没有招式／道具／特性），读不出结构签名。它们只按共现聚为一组，不是一个 archetype——共现是真实证据，结构是未知，而“未知”不等于“无任何结构”', "ja": '構造は未観測：これらの行は種族のみが公開された記録（技・持ち物・特性なし）で、構造シグネチャを読み取れません。共起だけでまとめた集合であり、アーキタイプではありません——共起は実データ、構造は不明であり、「不明」は「何も持たない」とは別の事実です', "en": 'STRUCTURE NOT OBSERVED: these rows are species-only records (no declared moves/items/abilities), so no structural signature could be read. They are pooled by co-occurrence alone and are NOT an archetype — the co-occurrence is real evidence, the structure is unknown, and unknown is not the same fact as "commits to nothing".'},
