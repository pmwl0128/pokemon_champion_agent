"""Capabilities handshake assembly (apps/design.md §3): one pass at startup over the four
...
# Must track packages/protocol WEB_PROTOCOL_VERSION — bumped 1 -> 2 for the breaking
...
def calc_engine_digest() -> str:
    ...
    build-time __ENGINE_DIGEST__ (apps/web/vite.config.ts calcEngineDigest), so the online runtime binds
